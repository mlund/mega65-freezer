...
unsigned char format_util_version(long addr, const unsigned char* date) {
    ...
    for (p = 0; p < 3; p++) {
        if (buffer[p * 2] >= '0' && buffer[p * 2] <= '9' && buffer[p * 2 + 1] >= '0' &&
            buffer[p * 2 + 1] <= '9') {
...
unsigned char get_rtc_stats(unsigned char reinit) {
    ...
    pa = ((tod_buf[1] >> 4) & 0x7) * 10 + (tod_buf[1] & 0xf) +
        (((tod_buf[2] >> 4) & 0x7) * 10 + (tod_buf[2] & 0xf)) * 60;
    pb = ((tod_buf[5] >> 4) & 0x7) * 10 + (tod_buf[5] & 0xf) +
        (((tod_buf[6] >> 4) & 0x7) * 10 + (tod_buf[6] & 0xf)) * 60;
    tod_ticks = pb - pa;
    if (tod_ticks < 0) {
    ...
    // handle RTC
    if (rtc_state & 1) {
        pa = ((rtc_buf[0] >> 4) & 0x7) * 10 + (rtc_buf[0] & 0xf) +
            (((rtc_buf[1] >> 4) & 0x7) * 10 + (rtc_buf[1] & 0xf)) * 60;
        pb = ((rtc_buf[6] >> 4) & 0x7) * 10 + (rtc_buf[6] & 0xf) +
            (((rtc_buf[7] >> 4) & 0x7) * 10 + (rtc_buf[7] & 0xf)) * 60;
        rtc_ticks = pb - pa;
        if (rtc_ticks < 0) {
